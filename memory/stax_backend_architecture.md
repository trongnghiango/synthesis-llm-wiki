...
- **Tenant Scope Enforcement:** In multi-tenant scopes, repository functions MUST check for `organizationId` or `tenantId` from the context or parameters via `applyTenantIsolation(conditions, table)`. Bảng `organizations` được lọc an toàn dựa trên chính khóa `id` của nó khi phát hiện `'organizationName' in table`. Bảng `finotes` được cô lập bằng cột `tenantId` when detecting `'tenantId' in table`.
- **Sub-module Isolation & Decoupling Rule:** Để triệt tiêu rủi ro vi phạm ranh giới (Domain Boundary) khi các module ngang hàng (như `CRM`) cần giao tiếp với sub-module con của một module lớn (như `Employee` của `HRM`):
  - **Quy tắc Vàng:** Chủ động tách biệt các sub-module có độ phủ sử dụng cao (`Employee`) ra thành các module độc lập đặt tại **Tier 2 (Domain Core)**. Điều này cho phép cả `HRM` và `CRM` gọi trực tiếp đến `Employee` mà không kéo theo sự phụ thuộc chéo vào các nghiệp vụ HRM phức tạp khác.
  - **Trường hợp đặc biệt (Dynamic Registry):** Chỉ khi không thể tách biệt do ràng buộc kỹ thuật sâu sắc, bắt buộc sử dụng **Register Pattern** (đăng ký các nhà cung cấp/handler tại thời điểm khởi tạo module, e.g., trong `SystemModule` hoặc `LookupRegistry`) để cấu hình động lúc runtime, tránh coupling cứng.
